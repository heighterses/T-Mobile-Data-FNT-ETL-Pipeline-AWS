...
def get_temp_table_schema():
    ...
    temp_tbl_name = "fnt.batch_temp_cardholder"
    sql0 = """CREATE SCHEMA IF NOT EXISTS fnt;"""

    #TODO columns/fields for batch_temp_cardholder need to be determined 
    sql1 =   """
            CREATE TABLE IF NOT EXISTS fnt.batch_temp_cardholder(
                
            );
            """
    sql2 =  """DELETE FROM fnt.batch_temp_cardholder"""
...
#TODO Reformat dates to YYYY-MM-DD
...
with cursor:
    try:
        ...
        cursor.execute("CALL upsert_dummy_cardholder();")
        conn.commit()
