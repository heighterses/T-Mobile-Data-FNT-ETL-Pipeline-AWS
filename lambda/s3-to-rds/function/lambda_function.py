"""
This Lambda function connects to Aurora Postgres and progresses ETL for tmus_fnt_finance schema
"""
...
def get_temp_table_schema(batch_file):
    ...
    if batch_file == 'cof-batch-transactions':
        temp_tbl_name = "tmus_fnt_finance.tbl_temp_cof_transactions"
        sql1 =   """
                CREATE TABLE IF NOT EXISTS tmus_fnt_finance.tbl_temp_cof_transactions(
                    creditCardLastFour TEXT,
                    surrogateAccountId TEXT NOT NULL,
                    customerId TEXT NOT NULL,
                    transactionEffectiveDate TEXT,
                    transactionPostingDate TEXT,
                    transactionPostingTime TEXT,
                ...
                """
        sql2 =  """DELETE FROM tmus_fnt_finance.tbl_temp_cof_transactions"""
        cols = ["creditCardLastFour","surrogateAccountId","customerId","transactionEffectiveDate","transactionPostingDate",
                "transactionPostingTime","authorizationDate","transactionCode","transactionType","transactionDescription",
                "dbCrIndicator","transactionReferenceNumber","internalExternalBrandFlag","transactionSequenceNumber",
                "authorizationCode","transactionAmount","transferFlag","transactionCurrencyCode","billedCurrencyCode",
                "conversionRate","merchantStore","merchantId","merchantCategoryCode","merchantName","merchantCity",
                "merchantState","merchantCountry","merchantPostalCode","invoiceNumber"]
    elif batch_file == 'cof-batch-account-summary':
        temp_tbl_name = "tmus_fnt_finance.tbl_temp_cof_account_summary"
        sql1 =   """
                CREATE TABLE IF NOT EXISTS tmus_fnt_finance.tbl_temp_cof_account_summary(
                ...
                """
        sql2 =  """DELETE FROM tmus_fnt_finance.tbl_temp_cof_account_summary"""
        cols = ["surrogateAccountId","nextPaymentDueDate","creditLimit","availableCredit","currentBalance",
                "nextStatementDate","lastPaymentDate","lastPaymentAmount","dateLastUpdated"]
    else:
        logger_function("ERROR: Unexpected error: S3 file name does not match required lookup.", type="error")
        sys.exit()
...
def lambda_handler(event, context):
    ...
    # (a) create tmus_financial schema if it does not exist
    ...
    try:
        ...
        bucket_name = "tmus-fnt-finance-stage"
        batch_file = "cof-batch-transactions"
        s3_file_path = "cof-batch-transactions.csv"
    except ClientError as e:
        logger_function("ERROR: Unexpected error: Could not access S3 file.", type="error")
        logger_function(e, type="error")
        sys.exit()

    sql1, sql2, temp_tbl_name = get_temp_table_schema(batch_file)
    ...
    cursor.close()
    conn.close()
    logger_function("SUCCESS: tmus_fnt_financial ETL operations complete.", type="info")
